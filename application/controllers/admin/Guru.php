...
class Guru extends CI_Controller
{
    ...
    function proses()
    {
        ...
        if (isset($post['insert'])) {
            ...
            $config['file_name']       = 'guru-' . $post['g_nuptk'];
            ...
            $post['id_user'] = random_string('numeric', 17);
            $this->guru_m->insert_guru($post);
            if ($this->db->affected_rows() > 0) {
                $this->session->set_flashdata('succes', 'Penambahan Data Guru Berhasil!,<br>Gunakan nuptk untuk login akun guru!');
                redirect('admin/guru');
            } else {
